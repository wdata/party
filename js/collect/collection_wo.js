...
var dropload = $('.heart').dropload({
    ...
    loadDownFn : function(me){
        $.ajax({
            type: "get",
            url:server_url + '//party-app-server/api/news/my/collection.json',
            dataType: 'json',
            data:{
            ...
            success:function(data){
                if(data.code === 0){
                    ...
                    var self = $(".wenzhang ul");
                    var html = '';
                    $.each(data.data.items,function(index,val){
                        html += '<li><a href="../friend/news.html?id='+ val.id +'"><img src="'+ unite_img_url +　val.image +'"/><div class="text"><header>'+ val.title +'</header><div class="time">'+ val.createTime +'</div></div></a></li>';
                    });
                    self.append(html);
...
function like(date){
    ...
    ajax_qcy(
        "get",
        '/api/ledgers/my/collection.json', {
            "userId":user_id.data.id,
            'date':date,
            "firmId":companyID,
            "page":0,
            ...
            if(data.code === 0){
                $.each(data.data.items,function(i,val){
                    ...
                    if(val.iscollects){
                        ...
                        shoucang = "shoucang.png"
                    }
                    var tag = '';
                    if(val.tagVOs){
                        tag = "<div class='title'>"+ val.tagVOs[0].name +"</div>";
                    }
                    html+="<li data-id='"+ val.id +"'  class='pic"+(i+1)+"'><a href='../templates/boao/show.html？id="+ val.id +"'><div class='title'>"+ val.title +"</div><img src='"+ unite_img_url + val.icon +"'/><i></i></a><div class='footer'><div class='like'><i class='icon'><img src='../images/collect/"+ shoucang +"' alt=''></i><span>"+ val.collects +"</span></div>"+ tag +"</div></li>";
                });
                self.html(html);
                $(".footer>.like>.icon").tap(function(){

                    var dian = "../images/collect/sc.png";
                    var zdd = "../images/collect/shoucang.png";

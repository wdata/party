...
(function($){
    $.fn.mdater = function(config){
        ...
        var mdater = {
            ...
            renderHTML : function(){
                var $html = $('<div class="md_panel"><div class="md_head"><div class="md_selectarea"><a class="md_prev change_month" href="javascript:void(0);">&lt;</a><a class="md_headtext yeartag" href="javascript:void(0);"></a><a class="md_headtext monthtag" href="javascript:void(0);">月</a> <a class="md_next change_month" href="javascript:void(0);">&gt;</a></div></div>');
            ...
            getDateStr : function(y, m, d){                                //根据日期更新日期列表
                ...
                for(var j=startDay; j<=endDay; j++){
                    ...
                    swiper_day+='<div class="swiper-slide">'+j+'</div>';
                }
                $("#swiper-wrapper").append(swiper_day);
                ...
                //    }
                //}
                console.log(swiper_day);
                return swiper_day;
            },
            updateDate : function(add){
                var date_swiper= $("#swiper-wrapper");//找到现有的滑动列表
                ...
                var newDateSwiper=$('<div class="swiper-wrapper '+c2+'"></div>');//更新滑动列表
                var newDateArea = $('<ul class="md_datearea '+c2+'"></ul>');//更新日期列表
                console.log(this.value.year+"  "+this.value.month+"  "+this.value.date);
                newDateSwiper.html(this.getDateStr(this.value.year, this.value.month, this.value.date));
                newDateArea.html(this.getDateStr(this.value.year, this.value.month, this.value.date));
                $(".date").append(newDateSwiper);
                //$('.md_body').append(newDateArea);
                setTimeout(function(){
                    newDateSwiper.removeClass(c2).addClass('in');
                    date_swiper.removeClass('in').addClass(c1);
                    newDateArea.removeClass(c2).addClass('in');//给新列表加in 这个class
                    dateArea.removeClass('in').addClass(c1);//给旧列表加c1这个class
                }, 0);
